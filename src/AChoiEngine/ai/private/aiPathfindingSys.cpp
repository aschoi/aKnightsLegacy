...
struct gridNode {
    int gNode_target_directDist = -1;     // Heuristic: direct distance from hunter to target
    int hunter_gNode_bestLocalDist = -1;  // actual best path distance from hunter to gridNode
    gridNode* parentNode = nullptr;
    std::vector<gridNode*> neighborNodes;
    int y_gu;
    int x_gu;
};

static int rawIntDist_gu(ACE_GameObject& hunter, ACE_GameObject& target) {
    float xDiff = target.get_x_gu() - hunter.get_x_gu();
    float yDiff = target.get_y_gu() - hunter.get_y_gu();
    float distFloat = std::sqrt((xDiff * xDiff) + (yDiff * yDiff));
    int distInt = static_cast<int>(std::round(distFloat));
    return distInt;
}
