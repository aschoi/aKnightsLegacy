...
bool Hud::Init(SDL_Renderer* r, float appW_pixels, float appH_pixels, ACE_Camera2D& cam, Player& p) {
    ...
    qCD_ = std::ceil(p.getSlide_remainCD());
    qCD_string = "Q CD: " + std::to_string(qCD_);
    wCD_ = std::ceil(p.getHammer_remainCD());
    wCD_string = "W CD: " + std::to_string(wCD_);
    eCD_ = std::ceil(p.getRollCD_remainCD());
    eCD_string = "E CD: " + std::to_string(eCD_);
    rCD_ = std::ceil(p.getProjectile_remainCD());
...
bool Hud::Init(SDL_Renderer* r, float appW_pixels, float appH_pixels, ACE_Camera2D_Center& cam, Player& p) {
    worldW_pixels = p.get_world_w_pixels();
    worldH_pixels = p.get_world_h_pixels();
    ...
    ACE_Cam_SetViewport(cam, appW_pixels_, appH_pixels_);
    return true;
...
void Hud::UpdateFrame(uint64_t now_ms, Player& p) {
    ...
    if (!(qCD_ == std::ceil(p.getSlide_remainCD()))) {
        qCD_ = std::ceil(p.getSlide_remainCD());
        qCD_string = "Q CD: " + std::to_string(qCD_);
        ACE_SetLineText(1, qCD_string);
    }
    if (!(wCD_ == std::ceil(p.getHammer_remainCD()))) {
        wCD_ = std::ceil(p.getHammer_remainCD());
        wCD_string = "W CD: " + std::to_string(wCD_);
        ACE_SetLineText(2, wCD_string);
    }
    if (!(eCD_ == std::ceil(p.getRollCD_remainCD()))) {
        eCD_ = std::ceil(p.getRollCD_remainCD());
        eCD_string = "E CD: " + std::to_string(eCD_);
        ACE_SetLineText(3, eCD_string);
    }
    if (!(rCD_ == std::ceil(p.getProjectile_remainCD()))) {
        rCD_ = std::ceil(p.getProjectile_remainCD());
        rCD_string = "R CD: " + std::to_string(rCD_);
...
void Hud::ACE_Render(SDL_Renderer* r, ACE_Camera2D_Center& cam) {
    for (ACE_HudLine& line : hudLines_) {
        if (!line.tex) continue;
