...
bool GameManager::Init(SDL_Renderer* appR, MIX_Mixer* appM, AudioManager* audioM, float appW_pixels, float appH_pixels, int tileSizeAsInt, float tileSizeAsFloat) {
    ...
    startMenuGS_->Init(appR, appW_pixels_, appH_pixels_, tileSizeAsInt_, tileSizeAsFloat_);
...
void GameManager::Shutdown() {
    ...
    if (startMenuGS_) {
        startMenuGS_->Shutdown();
        delete startMenuGS_;
        startMenuGS_ = nullptr;
    }
    if (mainGameGS_) {
        mainGameGS_->Shutdown();
        delete mainGameGS_;
        mainGameGS_ = nullptr;
    }
...
void GameManager::HandleEvent(const SDL_Event& event) {
    ...
    if (startMenuGS_ && curGS_ == GameState::StartMenu) {
        if (event.type == SDL_EVENT_KEY_DOWN && !event.key.repeat) {
            if (event.key.key == SDLK_UP) startMenuGS_->HandleEvent(Keys::Up);
            if (event.key.key == SDLK_DOWN) startMenuGS_->HandleEvent(Keys::Down);
            if (event.key.key == SDLK_RETURN || event.key.key == SDLK_KP_ENTER) {
                if (startMenuGS_->curOption == StartMenuOption::StartGame) {
                    startMenuGS_->HandleEvent(Keys::Confirm);
                }
        ...
        if (startMenuGS_->gameStartPressed == true) {
            ...
            creditsGS_->Init(appR_, appW_pixels_, appH_pixels_, tileSizeAsInt_, tileSizeAsFloat_);
        }
    }
    else if (mainGameGS_ && curGS_ == GameState::MainGameplay) {
        if (event.type == SDL_EVENT_KEY_DOWN && !event.key.repeat) {
            if (event.key.scancode == SDL_SCANCODE_SPACE) mainGameGS_->HandleEvent(Keys::Space);
            if (event.key.scancode == SDL_SCANCODE_Q) mainGameGS_->HandleEvent(Keys::Q);
            if (event.key.scancode == SDL_SCANCODE_W) mainGameGS_->HandleEvent(Keys::W);
            if (event.key.scancode == SDL_SCANCODE_E) mainGameGS_->HandleEvent(Keys::E);
            if (event.key.scancode == SDL_SCANCODE_R) mainGameGS_->HandleEvent(Keys::R);
...
void GameManager::UpdateFixed(double dt, const bool* kKeys) {
    if (startMenuGS_ && curGS_ == GameState::StartMenu) {
        startMenuGS_->UpdateFixed(dt, kKeys);
    }
    else if (mainGameGS_ && curGS_ == GameState::MainGameplay) {
        mainGameGS_->UpdateFixed(dt, kKeys);
    }
    else if (creditsGS_ && curGS_ == GameState::Credits) {
        creditsGS_->UpdateFixed(dt, kKeys);
    }
}
...
void GameManager::UpdateFrame(uint64_t now_ms) {
    ...
    if (startMenuGS_ && curGS_ == GameState::StartMenu) {
        startMenuGS_->UpdateFrame(now_ms);
    }
    else if (mainGameGS_ && curGS_ == GameState::MainGameplay) {
        mainGameGS_->UpdateFrame(now_ms);
    }
    else if (creditsGS_ && curGS_ == GameState::Credits) {
        creditsGS_->UpdateFrame(now_ms);
    }
}
...
void GameManager::Render(SDL_Renderer* appR) {
    ...
    if (startMenuGS_ && curGS_ == GameState::StartMenu) {
        startMenuGS_->Render(appR);
    }
    else if (mainGameGS_ && curGS_ == GameState::MainGameplay) {
        mainGameGS_->Render(appR);
    }
    else if (creditsGS_ && curGS_ == GameState::Credits) {
        creditsGS_->Render(appR);
    }
